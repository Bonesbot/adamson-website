...
def sr_variant(page, cfg):
    """The SiestaReport.com twin of a community page: same body, SRLayout instead of
    BaseLayout, JSON-LD and breadcrumbs pointing at siestareport.com, the contact CTA
    kept on-page, and the lead form pinned to the hub page path so lead-routing.js still
    sends Siesta Key leads to Ryan + Kelli and the source tag stays stable."""
    sr_url = cfg["sr_url"]
    out = page
    out = out.replace("import BaseLayout from '@/layouts/BaseLayout.astro';",
                      "import SRLayout from '@/layouts/SRLayout.astro';")
    out = re.sub(r"<BaseLayout (title=.*?) office=\"[^\"]*\" standalone>",
                 lambda m: f"<SRLayout {m.group(1)} canonicalUrl='{sr_url}'>", out, count=1)
    out = out.replace("</BaseLayout>", "</SRLayout>")
    out = out.replace(f"url: 'https://adamsonfl.com/siesta-key/{cfg['slug']}',", f"url: '{sr_url}',")
    out = re.sub(r"      \{ '@type': 'ListItem', position: 1, name: 'Home', item: 'https://adamsonfl.com/' \},\n"
                 r"      \{ '@type': 'ListItem', position: 2, name: 'Siesta Key', item: 'https://adamsonfl.com/areas/siesta-key' \},\n"
                 r"      \{ '@type': 'ListItem', position: 3, name: 'Gulf & Bay Club', item: 'https://adamsonfl.com/siesta-key/gulf-and-bay-club' \},\n"
                 r"      \{ '@type': 'ListItem', position: 4, name: [^\n]*\n",
                 "      { '@type': 'ListItem', position: 1, name: 'Home', item: 'https://siestareport.com/' },\n"
                 f"      {{ '@type': 'ListItem', position: 2, name: 'Gulf & Bay Club', item: '{sr_url}' }},\n", out, count=1)
    ...
    out = out.replace('<a href="/contact" class="gbc-btn gbc-btn-gold">Contact the Team</a>',
                      '<a href="#contact" class="gbc-btn gbc-btn-gold">Contact the Team</a>')
    out = out.replace("page: window.location.pathname,", f"page: '/siesta-key/{cfg['slug']}/',  // pinned: keeps lead routing + source tag on the hub path")
    out = out.replace("// GENERATED by scripts/gen_gulf_bay_pages.py:", "// GENERATED by scripts/gen_gulf_bay_pages.py (SiestaReport.com twin, see sr_variant):", 1)
    assert "adamsonfl.com" not in out.replace("adamsonfl.idxbroker.com", ""), "adamsonfl.com reference left in SR twin"
    assert "BaseLayout" not in out
    return out
...
def build_payload(conn):
    ...
    for key, cfg in SIDES.items():
        ...
        files[f"{REL_PAGES}/{cfg['slug']}.astro"] = page
        if cfg.get("sr_path"):
            files[cfg["sr_path"]] = sr_variant(page, cfg)
